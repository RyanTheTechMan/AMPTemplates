...
if ! grep -Fq 'LAUNCHER_TEMPLATE_VERSION="15"' "$BASE_DIR/azerothcore-run.sh"; then
    fail "Template/runtime version mismatch: installer v$INSTALLER_TEMPLATE_VERSION did not receive launcher v15. Update the configured template repository/ref before retrying."
fi
